...
def blight_model():
    ...
    y = pd.read_csv('test.csv',encoding = 'ISO-8859-1',low_memory=False)
    ...
    y.loc[:,'lat'].fillna(y.loc[:,'lat'].mode()[0],inplace=True)
    y.loc[:,'lon'].fillna(y.loc[:,'lon'].mode()[0],inplace=True)
    y.loc[:,'disposition'].fillna(y.loc[:,'disposition'].mode()[0],inplace=True)
    y.loc[:,'violation_code'].fillna(y.loc[:,'violation_code'].mode()[0],inplace=True)
    ...
    poly_feat_X_clean_data = poly.fit_transform(X_clean_data.iloc[:,X_clean_data.columns != "compliance"])
    
    poly_feat_y = poly.fit_transform(y)
    
    #X_clean_data = pd.DataFrame({'violation_code':X_clean_data[:,0],'disposition':X_clean_data[:,1],'compliance':X_clean_data[:,2],'lat':X_clean_data[:,3],'lon':X_clean_data[:,4]})
   
    X_train, X_test, y_train, y_test = train_test_split(poly_feat_X_clean_data,X_clean_data.loc[:,"compliance"], random_state=0,test_size=0.20)
    ...
    poly_feat_y = scaler.fit_transform(poly_feat_y)
    
    class_weights={0:1,1:0.2}
    knn=RandomForestClassifier (bootstrap = True, class_weight = class_weights, criterion = 'entropy',max_depth=14,  random_state = 0,n_estimators=350)
    ...
    #print("depth", depth, auc) 
    
    
    return pd.DataFrame(np.float32 (knn.predict_proba(poly_feat_y)[:,1]), y_index)
